...
interface Report {
  ...
  csv_sheets?: { name: string; headers: string[]; rows: string[][] }[];
  backlinks?: Backlink[];
...
function CsvSheetsView({ sheets }: { sheets: { name: string; headers: string[]; rows: string[][] }[] }) {
  ...
  const filtered = search
    ? sorted.filter(row => row.some(c => c.toLowerCase().includes(search.toLowerCase())))
    : sorted;
      ...
      <div className="px-5 py-4 border-b border-slate-100 flex flex-wrap items-center gap-3">
        <div className="flex-1">
          <h2 className="font-semibold text-slate-700 flex items-center gap-2">
            <BarChart3 size={16} className="text-blue-500" /> Data Sheets
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">{sheets.length} sheet{sheets.length !== 1 ? "s" : ""} attached</p>
        </div>
        {/* Sheet tabs */}
        <div className="flex flex-wrap gap-1.5">
          {sheets.map((s, i) => (
            <button key={i} onClick={() => { setActive(i); setSearch(""); setSortCol(null); }}
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-colors ${active === i ? "bg-blue-600 text-white border-blue-600" : "bg-white text-slate-600 border-slate-200 hover:border-blue-300"}`}>
              {s.name}
              <span className={`ml-1.5 ${active === i ? "text-blue-200" : "text-slate-400"}`}>{s.rows.length}</span>
            </button>
          ))}
        </div>
        {/* Search */}
        {sheet && (
          <div className="relative">
            <input value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search…"
              className="pl-7 pr-3 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-36" />
        ...
        <div className="overflow-x-auto max-h-[480px] overflow-y-auto">
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.slice(0, 300).map((row, ri) => (
                <tr key={ri} className={`transition-colors ${ri % 2 === 0 ? "bg-white" : "bg-slate-50/40"} hover:bg-blue-50/30`}>
              ...
              {filtered.length > 300 && (
                <tr><td colSpan={sheet.headers.length} className="px-3 py-2 text-center text-slate-400">
                  Showing 300 of {filtered.length} rows
                </td></tr>
              )}
              {filtered.length === 0 && (
                <tr><td colSpan={sheet.headers.length} className="px-3 py-8 text-center text-slate-400">No matching rows</td></tr>
              )}
            </tbody>
          </table>
        </div>
