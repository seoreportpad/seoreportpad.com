...
import {
  ...
  Share2, Bot, CheckSquare, Search, Languages,
} from "lucide-react";
...
import { CanonicalCheckerTool } from "@/components/tools/CanonicalCheckerTool";
import { SerpPreviewTool } from "@/components/tools/SerpPreviewTool";
import { HreflangGeneratorTool } from "@/components/tools/HreflangGeneratorTool";

type ToolId =
  | "meta" | "title" | "robots" | "sitemap" | "redirect" | "wordcount" | "slugify" | "utm"
  | "ogpreview" | "robotstester" | "canonical" | "serppreview" | "hreflang";

const TOOLS: { id: ToolId; label: string; desc: string; icon: React.ElementType; color: string; isNew?: boolean }[] = [
  ...
  { id: "ogpreview",    label: "OG / Social Preview",     desc: "Preview how your page looks on Facebook & Twitter",      icon: Share2,       color: "bg-sky-500",    isNew: true },
  { id: "robotstester", label: "Robots.txt Tester",       desc: "Test if a URL is blocked or allowed by robots.txt",      icon: Bot,          color: "bg-indigo-500", isNew: true },
  { id: "canonical",    label: "Canonical Checker",       desc: "Verify canonical tags — self, cross-domain, missing",    icon: CheckSquare,  color: "bg-emerald-500", isNew: true },
  { id: "serppreview",  label: "SERP Snippet Preview",    desc: "Preview your Google search result before publishing",    icon: Search,       color: "bg-rose-500",   isNew: true },
  { id: "hreflang",     label: "Hreflang Generator",      desc: "Generate hreflang tags for multilingual SEO",            icon: Languages,    color: "bg-purple-500", isNew: true },
];
...
const TOOL_COMPONENTS: Record<ToolId, React.ComponentType> = {
  ...
  slugify: SlugifyTool,
  utm: UtmTool,
  ogpreview: OgPreviewTool,
  robotstester: RobotsTesterTool,
  canonical: CanonicalCheckerTool,
  serppreview: SerpPreviewTool,
  hreflang: HreflangGeneratorTool,
};
    ...
    <div className="animate-fade-in max-w-6xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">13 professional utilities to optimize your search presence</p>
        </div>
      </div>
        ...
        <>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {TOOLS.map(tool => (
              ...
              >
                {tool.isNew && (
                  <span className="absolute top-3 right-3 text-xs bg-blue-100 text-blue-700 font-bold px-1.5 py-0.5 rounded-full">NEW</span>
                )}
