...
import {
  competitorSopData,
  getCompOverallProgress,
  getCompCategoryProgress,
} from "@/lib/competitorSopData";

const STORAGE_KEY = "competitor-sop-checked";
...
export default function CompetitorSopPage() {
  const [checked, setChecked] = useState<Record<string, boolean>>({});
  const [activeTab, setActiveTab] = useState(competitorSopData[0].id);
  ...
  const overall = useMemo(() => getCompOverallProgress(checked), [checked]);
  const overallPercent = overall.total > 0 ? Math.round((overall.done / overall.total) * 100) : 0;

  const activeCategory = competitorSopData.find((c) => c.id === activeTab)!;
  const catProgress = getCompCategoryProgress(activeTab, checked);
  ...
  const mustLeft = activeCategory.checks.filter(
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-5">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-2xl font-bold text-gray-900">Competitor Research SOP</h1>
            ...
            <p className="text-sm text-gray-500">
              {overall.total} checkpoints — competitor analysis ka complete framework
            </p>
          </div>
        ...
        <div className="max-w-7xl mx-auto mt-3 flex items-center gap-4 text-xs">
          ...
          <span className="bg-red-100 text-red-700 border border-red-200 px-2 py-0.5 rounded-full font-semibold">Must Do</span>
          <span className="bg-yellow-100 text-yellow-700 border border-yellow-200 px-2 py-0.5 rounded-full font-semibold">Should Do</span>
          <span className="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-full font-semibold">Nice to Have</span>
        </div>
      </div>
        ...
        <aside className="lg:w-64 shrink-0">
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden sticky top-4">
            <div className="px-4 py-3 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider">
              Analysis Steps ({competitorSopData.length})
            </div>
            <div className="overflow-y-auto max-h-[calc(100vh-200px)]">
              {competitorSopData.map((cat, idx) => {
                const prog = getCompCategoryProgress(cat.id, checked);
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs font-medium leading-tight ${isActive ? "text-orange-700" : "text-gray-700"}`}>
                          {cat.title}
                        </span>
                      </div>
                      <span className={`text-xs font-bold shrink-0 ${pct === 100 ? "text-green-600" : isActive ? "text-orange-600" : "text-gray-400"}`}>
                        {pct}%
                      </span>
                    </div>
        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <span className="text-sm text-gray-600">
                      <span className={`font-bold ${catPercent === 100 ? "text-green-600" : "text-orange-600"}`}>
            ...
            <div className="mt-4 h-2.5 bg-gray-100 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-500 ${catPercent === 100 ? "bg-green-500" : "bg-orange-500"}`}
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4 flex flex-wrap gap-3 items-center">
            <input
              ...
              className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-300 flex-1 min-w-[180px]"
            />
          ...
          {catPercent === 100 && (
            <div className="mt-4 bg-green-50 border border-green-200 rounded-xl px-5 py-4 text-center text-green-700 font-semibold text-sm">
              ✅ {activeCategory.title} — Complete! Agla step karo.
            </div>
          )}

          {overallPercent === 100 && (
            <div className="mt-4 bg-orange-50 border-2 border-orange-300 rounded-xl px-5 py-5 text-center">
              <div className="text-2xl mb-2">🏆</div>
              <p className="text-orange-800 font-bold text-lg">Competitor Analysis Complete!</p>
              <p className="text-orange-600 text-sm mt-1">Ab report banao aur client ko present karo.</p>
            </div>
          )}
        </main>
      </div>
